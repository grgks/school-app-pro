...
import gr.aueb.cf.schoolapp2.dto.BaseStudentDTO;
import gr.aueb.cf.schoolapp2.dto.BaseTeacherDTO;
...
public class StudentValidator <T>{
    ...
    public static <T extends BaseStudentDTO> Map<String, String> validate(T dto) {
        ...
        if (dto.getFatherName().length() < 2 || dto.getFatherName().length() > 32) {
            errors.put("fathernane", "Το όνομα Πατρός πρέπει να είναι μεταξύ 2 και 32 ψηφία");
        }

        if (dto.getFatherName().matches("^.*\\s+.*$")) {
            errors.put("fathernane", "Το όνομα Πατρός δεν πρέπει να περιλαμβάνει κενά");
        }
